const http = {
    ...
    ajax: (settings = {}) => {
        ...
        const s = { 
            ...
            headers: {}, // object 请求头
            timeout: 15000, // string 超时时间:0表示不设置超时
            beforeSend: xhr => {
                console.log(xhr)
            },
        ...
        // 参数验证
        if (!s.url || !s.type || !s.dataType || !s.async) {
            console.log('参数有误')
            return
        }
        ...
        if (sType === 'GET' || sType === 'DELETE') {
            useUrlParam = true
            s.url += http.getUrlParam(s.url, s.data)
        }
        // 初始化请求
        ...
        // 设置请求头
        for (const key of Object.keys(s.headers)) {
        ...
        xhr.send(useUrlParam ? null : http.getQueryData(s.data))
    },
    // 把参数data转为url查询参数
